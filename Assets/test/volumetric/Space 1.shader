...
Shader "Custom/perlin" {
	Properties {
		...
		_StarThreshold("Star threshold", Range(0., 1.)) = 0.85
	}
	SubShader {
		...
		Pass
        {
			...
			fixed4 raycast(float3 position, float3 direction)
			{
				...
				for(int i = 0; i < STEPS; i++){
					localDensity = cnoise(position * _Scale);
					localStarDensity = fBm_F0(position * _StarScale, OCTAVES);
					value += localDensity * _Intensity * stepLength;
					if(localDensity > _StarThreshold)
						break;
					
					position += direction;
				}
				return 1-2*length(position);
			}
			...
			fixed4 frag (v2f i) : SV_TARGET
			{
				...
				fixed4 col = raycast(position, viewDirection) * _Color;
